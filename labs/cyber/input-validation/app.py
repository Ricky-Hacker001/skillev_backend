...
<head>
    ...
    <style>
        ...
        .btn { width: 100%; padding: 12px; background: #3b82f6; color: black; font-weight: bold; border: none; border-radius: 8px; cursor: pointer; }
    </style>
</head>
<body>
    {% if mode == 'learning' %}
    <div class="sidebar">
        ...
        <div class="logic-step">
            ...
            <p>The form uses HTML5 attributes like <code>maxlength="10"</code> to limit your input. This is for UI/UX, not security.</p>
        </div>
        <div class="logic-step">
            <b>02. Bypassing the UI</b>
            <p>Intercept the request using a proxy or manipulate the DOM to send a payload longer than the allowed 10 characters.</p>
        </div>
    </div>
    {% endif %}
    ...
    <div class="main-content">
        <div class="container">
            ...
            <form method="POST" id="updateForm">
                ...
                <input type="text" name="display_name" id="displayName" maxlength="10" placeholder="Ricky" required>
                <button type="submit" class="btn">Update Protocol</button>
            </form>
            {% if message %}<div style="margin-top:20px; color: #10b981;">{{ message | safe }}</div>{% endif %}
        </div>
    </div>
</body>
</html>
...
def index():
    ...
    if request.method == 'POST':
        ...
        # The bypass occurs here because the server doesn't re-verify length
        if len(name) > 10:
            result_msg = "SUCCESS: Validation_Bypass_Confirmed"
            print(f"EVIDENCE_LOG: {result_msg}", flush=True)
            message = f"<b>{result_msg}</b><br>Updated to: {name}"
        else:
